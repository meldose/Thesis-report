...
2. Use PhoXi Control's marker pattern to save calibration \\\\
 For the setting we can use it as a custom scanning profile in VS Space by adding a triggering item for the sensor at the marker pattern export photo's origin.
...
\subsection{Objects for picking}
...
The below figure shows the results that is obtained while pciking the items in an moving conveyor belt (m/s) wiht respect to the Success Rate.
\label{Literature}
\begin{figure}[h]
    \centering
    \includegraphics[width=16cm]{Thesis-report/Figures/IMG_6355.PNG} 
