...
\begin{enumerate}
    ...
    \item Calculate the current time and update the position X with the equation ([X=X0 + velocity x t])
...
Many applications in manufacturing, construction, and vehicle automation and autonomy depend on the availability of 3D information.  Time-of-flight (ToF) sensors are getting cheaper and more available. They offer depth information for every pixel at every pixel in addition to intensity. The ToF sensor provides spherically-coordinated range data of the environment within its field of view.  As described in the following section, the spherical coordinates are transformed into Cartesian coordinates, with the origin of the coordinate system located at the camera's center.
 ...
 \end{enumerate}
\subsection{Disadvantates of gimball lock and Ambiguity}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{Thesis-report/Figures/conveyor_speed vs Robot tcp speed.png}
  \caption{Mean TCP speed of the robot versus conveyor belt speed.}
  \label{fig:tcp_vs_conveyor}
\end{figure}

\section{Success Rate vs Conveyor Speed}
Figure~\ref{fig:success_vs_conveyor} shows the success rate for dynamic picking of trapezoid and pipe objects at belt speeds between 1.0\,m/s and 2.0\,m/s. During the dynamic picking tests, the robot maintained a higher success rate on trapezoid objects compared to pipes.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{Thesis-report/Figures/conveyor_speed vs success_rates.png}
  \caption{Success rate of dynamic picking versus conveyor belt speed.}
  \label{fig:success_vs_conveyor}
\end{figure}

\section{Average Pick Time vs Conveyor Speed}
The average pick time per item is plotted in Figure~\ref{fig:picktime_vs_conveyor}. The red line with circular markers shows that pick times increase from approximately 0.5\,s at 1.0\,m/s to 1.5\,s at 2.0\,m/s, indicating that higher belt speeds result in longer pick durations.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{Thesis-report/Figures/conveyor_speed vs avg pick_up_time.png}
  \caption{Average item-pick time versus conveyor belt speed.}
  \label{fig:picktime_vs_conveyor}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{Thesis-report/Figures/Picking_time vs item_type.png}
  \caption{Average pick time and success rate by item type under different lighting conditions.}
  \label{fig:picktime_vs_lighting}
\end{figure}
