...
\begin{itemize}
    ...
    \item Movelinear
\end{itemize}\\
1. \textbf{ServoX:} In the Servo X motion, we move the robot in cartesian motion, where the coordinates that we get from the camera are in quaternion poses [X, Y, Z, Qx, Qy, Qy, W]. These are already in the cartesian poses, which can be directly passed to the robotic controller and give these values as the target position and move the robot to the desired location. During the servoX , the coordiantes that got from the camera were not in the correct order as that of the robotic movement, so initially had to change the target position as in this form [X, Y, Z, W, Qx, Qy, Qy]\\\\
2. \textbf{ServoJ:} In the servo J , the move of the robot is with the joint movements, instead of cartesian. The object coordinates that we get from the camera are in Cartesian, they need to be converted into joint form and given as the target position. When giving the target position, the final orientation values of the coordinates will not be the same as that of the TCP coordinates of the robot. In order to set this, we have to implement the method inverse kinematics.\\\\
3. \textbf{Movelinear:}
The Movelinear is the method where the target postion for the robot to pick up the object is directly given as the quaternion values provided by the camera by scanning the objects.
\newpage
\subsection{Static Picking:}\\
The static picking is the process of picking the items from the conveyor belt when it is not moving. Initially we keep the object in the conveyor belt, then trigger the camera that helps in getting the object coordinates to the robotic controller. When the robot recieves the data , the robot move to the target position for picking the item required target postion.
\subsection{Dynamic Picking:}\\
The Dynamic picking involves the process of picking the items from the moving conveyor belt . Here the object is passed through the moving conveyor bet where the camera get object poses that are passed to the robotic controller, that helps the robot to move to the required positon.
