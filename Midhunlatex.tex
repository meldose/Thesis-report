...
This camera can capture accurate point clouds and a standard intensity image of the object. Its foundation is a specialized CMOS image sensor that uses Photoneo's patented Parallel Structured Light technology.\\
\begin{figure}[h]
    \centering
...
Early work on visually guided conveyor tracking focused on 2D cameras and simple background‐subtraction methods. Shin AUTHOR NAME Demonstrated frame‐difference object detection combined with 2D template matching to locate parts on a belt in real time, but suffered from occlusion and uniform‐lighting limitations: REFERENCE TO. Park and Lee formulated the conveyor‐tracking problem as a constrained trajectory‐planning task for 6‐DOF manipulators, minimizing steady‐state tracking error and actuator torques: REFERENCE TO. More recently, Zhang AUTHOR NAME integrated Time‐of‐Flight ranging with color segmentation to classify and track parcels at higher belt speeds, achieving sub‐5 mm positional accuracy at 1 m/s: REFERENCE TO \\\\ 

\subsection{3D Vision Systems and Depth Sensing}
The shift from 2D to 3D sensing has been driven by affordable structured light and TOF cameras. Photoneo’s MotionCam 3D M+ uses Parallel Structured Light to capture 1068 × 800 depth frames at 60 Hz, merging per‐pixel depth with intensity for robust object detection under ambient light: REFERENCE TO. Fernandez AUTHOR NAME showed how TOF depth data can be converted from spherical to Cartesian coordinates for conveyor‐based pick‐and‐place, with typical z‐noise under 0.5 mm at 1 m distance: REFERENCE TO. Luo AUTHOR NAME extended this by fusing stereo‐vision point clouds with TOF to improve tracking through partial occlusions: REFERENCE TO.\\

\newpage
\subsection{Calibration Techniques (Hand–Eye and Extrinsic)}\\
Accurate hand-eye and extrinsic calibration is critical for mapping camera coordinates into the robot base frame. Standard methods use a calibration rig (ball or checkerboard) and solve for the transform ${}^{\rm base}H_{\rm cam}$ via least‐squares on multiple correspondences: REFERENCE TO. Ge AUTHOR NAMEautomated lidar–camera extrinsic calibration with a hemispherical target, yielding sub‐millimeter accuracy over nine poses: REFERENCE TO. Photoneo’s web interface further streamlines extrinsic setup by guiding users through nine sample points and reporting an overall accuracy score in millimeters. REFERENCE TO

\subsection{Dynamic Object Tracking and Path Planning} Real-time tracking of moving parts requires coupling vision updates with trajectory replanning. Shahzadi AUTHOR NAME proposed a vision‐based path planning framework where each new depth frame triggers a local replanning via a fast pseudo‐inverse Jacobian update, ensuring collision‐free, smooth motion: REFERENCE TO. Wunsch and Hirzinger introduced dynamic occlusion handling in 3D by predicting object pose through Kalman filtering of multi‐view point clouds: REFERENCE TO. Agrawal AUTHOR NAME Demonstrated shadow‐casting to enhance grasp edges, reducing processing latency in high‐speed pick‐and‐place: REFERENCE TO.
...
where: camera Pobj: [u, v, 1]" on the image plane; Pobj: The object point [EX, CY, CZ]" in the camera frame.
 ...
 The coordinate conversion from the stereo camera frame to the robot base frame is the second operation sequence.  The transformation matrix base Hcam, which is determined from the eye-to-hand calibration, is used to do this.  Combining the first and second operation sequences yields the equation:\\
...
\begin{figure}[h]
    ...
    \caption{Calibration for camera concerning Robot frame}[2] 
    \label{fig1.Photoneo Cmaera}
\end{figure}
...
\newpage
Figure 18 below shows the calibration method used in this study (Extrinsic Calibration) (EC), where the camera is fixed at the location, and 9 points are taken into consideration, where the average value is finally taken as the value for the calibration. [2]\\
...
Initially we need to calculate the pose of the object that need to be picked up
...
\begin{enumerate}
    ...
    \item Use PhoXi Control's marker pattern to save calibration. We can use this setting as a custom scanning Mrile in Vision System´s Space by adding a triggering item for the sensor at the marker pattern export photo's origin.[21]
...
\begin{itemize}
  \item $\Delta TSC$ is the \textbf{Target Scan Completion Distance}%
        —distance moved by the conveyor during scan completion.
  \item $\Delta CTO$ is the \textbf{Conveyor Tracking Offset}%
        —distance moved while waiting for the robot to reach the pick position.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Second step:} Switch on the conveyor belt and get the farthest position where the camera triggers the object, and get the coordinates which includes [X2, Y2, and Z3] respectively.[2]
...
The system model, which includes a non-redundant robot manipulator with n degrees of freedom and a conveyor belt system. The following attributes are assumed to be present in the system model: [14]\\
...
\begin{description}
  ...
  \item[$\dot{\theta}$]%
    ...
    \[
      ...
      \end{bmatrix},[19]
    \] 
    with units: rad/s for revolute, m/s for prismatic joints.
...
This refers to the workpiece or tracked objects that travel along the conveyor belt. In certain applications, part orientation plays a crucial role in assembly procedures. In our application, we just want to choose or select the objects, independent of their orientation; thus, this isn't accurate.\\
 ...
 This refers to the location of the robot's end-tool tip. As you can see, a robotic hand typically has a mechanism attached to the end, with a gripper. When referring to the robot base frame, this typically requires a little amount of offset and aids the robot in completing its responsibilities.\\\\
...
There are two software components which includes:
\begin{enumerate}
...
\begin{itemize}
  \item \textbf{Loss of One Degree of Freedom.} 
    When two of the three rotation axes become aligned (for example, in a Z–X′–Z sequence the X′ axis lines up with Z), one “gimbal” loses its ability to rotate independently. Consequently, the local parameter space collapses from three dimensions to two.[20]
  \item \textbf{Numerical Instability.} 
    Near the singular alignment (e.g.\ a pitch of \(\ pm90^\circ \) in a Z–Y′–X sequence), small orientation changes require large swings in two of the Euler angles, amplifying floating-point errors.[20]
  \item \textbf{Control \& Interpolation Problems.} 
    In applications such as flight control or animation, smoothly interpolating through a gimbal-lock singularity can cause sudden flips or erratic motion.[20]
\end{itemize}

\subsubsection{2. Ambiguity (Non-Uniqueness)}
\begin{itemize}
  \item \textbf{Multiple Angle Triples for the Same Rotation.} 
    Because each rotation about an axis is periodic (modulo \(360^\circ\)), and because different sequences can reach the same final orientation, there are infinitely many \((\alpha,\beta,\gamma)\) that yield the same rotation matrix. For instance, a “roll” of \(+360^\circ\) is indistinguishable from \(0^\circ\).[20]
  \item \textbf{Convention Dependence.} 
    There are twelve common Euler conventions (proper Euler vs.\ Tait–Bryan, intrinsic vs.\ extrinsic, and choice of axis ordering). Without knowing the exact convention, an \((\alpha,\beta,\gamma)\) triplet is ambiguous.[20]
  \item \textbf{Inverse-Mapping Ambiguities.} 
    Given a rotation matrix, solving for Euler angles often yields two valid solutions (sometimes called the “standard” and “supplementary” sets), requiring extra logic to choose one.[20]
\end{itemize}
\newpage
\subsubsection*{Why It Matters}
\begin{itemize}
  \item \textbf{Robustness:} Gimbal lock can cripple systems such as aircraft autopilots, robotic manipulators, or VR head-tracking.[20]
  \item \textbf{Maintainability:} Ambiguity forces code, file formats, and APIs to rigidly specify which Euler convention is used to avoid misinterpretation.[20]
\end{itemize}

\subsubsection*{Alternatives}
Many applications prefer other orientation representations to avoid both gimbal lock and ambiguity:
\begin{itemize}
  \item \textbf{Quaternions.}  Free of singularities, supports smooth spherical linear interpolation (slerp), unique up to a sign.[20]
  \item \textbf{Rotation Vectors / Axis–Angle.}  One-to-one mapping (aside from sign and \(2\pi\) periodicity).[20]
  \item \textbf{Rotation Matrices.}  Unambiguous and globally non-singular, though over-parameterized (9 numbers with 6 constraints).[20]
\end{itemize}

\subsubsection{Rotation matrices}\\
Euler angles are usually implemented using rotation matrices. There is an x rotation matrix, a y rotation matrix, and a z rotation matrix for every kind of roll. The position vector for the rotated point is obtained by multiplying the matrices by the position vector for a point in space. Although a rotation matrix is a 3 × 3 matrix, homogeneous 4 × 4 matrices are typically utilized in its place. The three roll matrices corresponding to the three Euler angles are multiplied to produce a general rotation. The generated matrix can be applied to the locations that need to be rotated and represents the general rotation. In most cases, matrix multiplication is not commutative. The fact that rotations in space do not commute fits in nicely with this. Lastly, it should be mentioned that the only implementation that effectively incorporates all common transformations—translation, scaling, shearing, and different projection transformations—is the use of homogeneous transformation matrices. s.[20]
...
Rotations in 3D can be implemented using Euler’s theorem combined with \emph{quaternions}, an extension of complex numbers.[20]
...
The above figure shows the volume/area in which the Photoneo Camera can scan items and determine object poses. The top view shows the maximum field of view (FOV) reachable by the camera for object coordinate acquisition, which extends up to 15 m.The camera sits around 350mm tall and projects a pyramidla field of view with colored ones. The Depth lanes at 630 mm,907mm and 1574mm from sensor; at ecah distance the intersection of the FOV with flat plan is a rectangle that grows with distance.Moreover, the blue trapezoid is the maximum rectangular cross section of the scan volume with sensor horizontal, vertical, and diagonal field of view limits.

\newpage
\chapter{\section{\mbox{Results and Discussions }}{{\normalfont\fontsize{14}{16}\bfseries}}
\section{Robot TCP Speed vs Conveyor Speed}
The robot's tool-center-point (TCP) speed is plotted against the belt speed in  The red circles show the measured TCP speeds, whereas the blue line represents the identity (conveyor speed). The robot's mean TCP speed increases approximately linearly from 0.08\,m/s to 0.31\,m/s as the belt speed increases from 1.00\,m/s to 2.00\,m/s, constantly trailing the conveyor.
\begin{figure}[h]
    \centering
...
Figure below shows the success rate for dynamic picking of trapezoid and pipe objects at belt speeds between 1.0\,m/s and 2.0\,m/s. During the dynamic picking tests, the robot maintained a higher success rate on trapezoid objects compared to pipes.
...
The average pick time per item is plotted in the Figure below. The red line with circular markers shows that pick times increase from approximately 0.5\,s at 1.0\,m/s to 1.5\,s at 2.0\,m/s, indicating that higher belt speeds result in longer pick durations.
...
Figure~\ref{fig:picktime_vs_lighting} contrasts the robot's picking performance under bright and dim lighting for pipes and trapezoids. Under bright light, the success rates are 80\% for pipes and 90\% for trapezoids. In dim light, success drops to 60\% for pipes and 80\% for trapezoids, showing that dimmed illumination affects pipe handling more severely.
...
The smooth movement of items through several stages of manufacturing, storage, and distribution is made possible by conveyor tracking systems, which are now essential parts of contemporary industrial and logistical processes.  Technological developments throughout time have greatly increased these systems' precision, dependability, and efficiency.  Among the major advancements in conveyor tracking are:

\begin{itemize}

\item Improved Automation and Integration: Conveyor tracking has expedited processes, decreased manual intervention, and minimized errors when integrated with other automated systems, such as Enterprise Resource Planning (ERP) software and Warehouse Management Systems (WMS).

 \item Real-Time Monitoring and Data Analytics: To give real-time information on the flow of items, contemporary conveyor tracking systems make use of sensors, Internet of Things devices, and sophisticated analytics.  Proactive decision-making, predictive maintenance, and efficient resource allocation are made possible by this skill.

\item Increased Efficiency and Accuracy: Advances in tracking technology, such as barcode scanners, RFID tags, and machine vision, have greatly increased the precision of item identification and tracking, which has decreased product theft, loss, and misplacement.

\item Flexibility and Scalability: Modern conveyor systems are made to be extremely flexible and scalable, enabling companies to modify their operations in response to shifting market conditions and demand without having to make significant infrastructural changes.

\item Sustainability & Energy Efficiency: Energy-saving motors, regenerative braking systems, and environmentally friendly materials have all been included in conveyor technology advancements to lessen the environmental impact of conveyor operations.
\end{itemize}
\newpage
...
\item Artificial Intelligence and Machine Learning: Combining AI and ML algorithms can improve system intelligence overall, optimize routing, and strengthen predictive maintenance.  Large volumes of data may be analyzed by these technologies to spot trends, forecast breakdowns, and suggest real-time operational changes.

\item Advanced IoT Integration: As IoT devices proliferate, conveyor tracking systems' connectivity and interoperability will be further improved.  More detailed monitoring, better data collecting, and smooth communication between various systems and devices will all be made possible by enhanced IoT integration.

\item Virtual reality (VR) and augmented reality (AR) have the potential to completely transform maintenance, training, and operational planning.  In order to decrease downtime and increase productivity, maintenance workers can, for example, wear AR glasses to receive real-time guidance and overlays when servicing conveyor systems.

\item Blockchain for Improved Security and Transparency: By putting blockchain technology into practice, it is possible to create unchangeable records of product handling and movement, which will improve supply chain traceability, security, and transparency.

\item Automation and Robotic Integration: Material handling procedures can be further automated by combining robotics with conveyor tracking systems.  Conveyor systems and collaborative robots (cobots) can work together to complete activities like sorting, packaging, and inspection more quickly and precisely.

\item Green technologies and sustainability: In the future, conveyor systems will progressively integrate sustainable practices like energy-efficient designs, recyclable materials, and the utilization of renewable energy sources.  Waste reduction and environmental effect reduction innovations will become critical.

\item Improved user interfaces and human-machine interaction: Operators and conveyor tracking systems will communicate more effectively as a result of the creation of more intuitive and user-friendly interfaces.  Touchless controls, voice-activated interfaces, and sophisticated visualization tools will increase the efficiency and accessibility of system management.

\item Edge Computing and Improved Processing Capabilities: By putting edge computing into practice, data processing may take place closer to the source, which lowers latency and boosts conveyor tracking systems' responsiveness.  This will be especially helpful for areas with poor connectivity and real-time applications.

\item Customization and Modular Designs: More customization and flexibility will be available in future conveyor tracking systems, enabling companies to adapt systems to their unique requirements without having to completely redesign them.  It is simple to add, delete, or reorganize modular components to accommodate shifting operational needs.

\item Improved cybersecurity measures: It will be crucial to have strong cybersecurity as conveyor tracking systems grow more interconnected and dependent on digital technologies.  In order to defend against cyber dangers, future advances will concentrate on putting improved security protocols, encryption techniques, and intrusion detection systems into place.
\end{itemize}
