...
\begin{figure}[h]
    ...
    \caption{Web interface for Photoneo}
    \label{fig1.Photoneo Cmaera}
\end{figure}
...
We look at the system model that includes a nonredundant robot manipulator with n degrees of freedom and a conveyor belt system.The following attributes are presumed to be present in the system model:\\
1) The conveyor belt system runs at the constant velocity, and the
part is stationary with respect to the conveyor belt system.\\
2) The orientation of the robot hand is aligned with the part
initially and kept fixed during the robot tracks the part.\\
...
\begin{figure}[h]
    ...
    \label{fig1.Photoneo Cmaera}
\end{figure}
The robot motion to be designed must meet certain conditions in order to maximize the manipulator's useable performance on conveyor tracking. These requirements can be multiplied as follows to create a single differential equation.\\
...
Sixteen alternatives are available for a general six DOF robot, as shown below, and it lists the quantity of analytical solutions for robots with varying degrees of freedom. Therefore, an accurate set of solutions among several inverse kinematic solutions is needed for the continuous motion of a robot. Jacobian inverse is another technique for resolving IK problems [5, 6]. In [7, 8], the numerical approach to solve the Jacobian matrix's inverse was introduced. In [9], the inverse kinematics problem for a six-DOF robot manipulator is solved analytically. The benefit of analytical expression is that it provides formulas for the connection between link parameters and joint angle. The robotics controller can directly incorporate these relationships.\\
...
The change in the end-effector's pose that corresponds to the change in joint angles θ is denoted by the word x e. The recursive relationship between the angular velocity and linear velocities for a 6R manipulator yields the Jacobian J, which is the 6×6 matrix for a 6-DOF robot. It is provided as \\
...
where a 1 is the joint axis direction of joint 1 and e is the vector as seen in Figure 2 (a). Likewise, direction and (12) e 1 is the joint axis a 2, e is the EE's location from joint 2, and so forth. The differential rotation and translation vectors that correspond to the differential change in the joint rates make up each column of the Jacobian. The Jacobian inverse provides the joint variation for the desired increment at the kth point pose as follows:\\
\[
\Delta \theta_k = J_k^{-1} \Delta x_e
\]
where x e represents the required increase in the EE's posture. In [13], the effective method for determining the increment was provided. Joint angles needed to get to the kth point are comparable to the joint angle's first-order Taylor series expansion, which is calculated as: \\
...
J = J (JJ)
\]
 is the null space of J with ⁄φ as the objective function, and J = J (JJ) -1 is the pseudo-inverse of the matrix J.x J.
Since the Jacobian matrix is square and invertible, (14) is utilized in this case for the manipulator with DOF = 6.
\newpage
...
A coordinate system is often specified by the user or developer, but it can exist anywhere in the world. As long as WCS makes it simpler to explain the locations of other things or items in that specific area, you can basically put it anywhere in your room or factory. Origin placement is typically done at the edge of a group or at a room's corner.\\
 \textbf{Machine Coordinate System (MCS):} \\
Robot Base Frame Coordinate System is another name for Machine Coordinate System (MCS).All other coordinate systems will be compared to this most significant coordinate system. Where developers and programmers would implement their codes based on this coordinate system is equally crucial. MCS is typically positioned near the robot's base for the point of origin, though it's important to remember that different robot types have different bases. For instance, the base of a SCARA robot is at the bottom, whereas the base of a Delta robot is in the upper center.\\
 \textbf{Coordinate System for Parts and Workpieces (PCS):} \\
referring to the workpiece or tracked objects that travel along the conveyor belt. In certain applications, part orientation plays a crucial role in assembly procedures. In our application, we just want to choose or suction the foreign objects, independent of their orientation; thus, this isn't accurate.\\
...
The orientation of the conveyor belt coordinate and the camera origin coordinate deviate from one another. The origin for a camera is the upper left corner of its field of vision (FOV), where the values of its pixels start at (0, 0). This origin is set in stone and cannot be altered. For the conveyor belt, we can freely provide the origin's location, but the standard procedure is to first specify the picking window's dimensions (it's a square), taking into account the robot's picking range limit, and then place the origin at the center of the window's bottom.\\
...
\begin{figure}[h]
    ...
    \label{fig1.Photoneo Cmaera}
\end{figure}
As can be seen from the above image, in order to match the conveyor belt origin, we must perform translation transformation in both the X and Y axes. We must do a rotation transformation for the Z-axis since the belt origin faces the other direction from the camera origin. In summary, this section contains two transformations: Rotation transformation, Rot(x, 180°); AND Translation in X-axis and Y-axis, Trans(ΔX, ΔY, 0). Observe that both the Y and Z axes changed their facing directions when we rotated the X axis. For this section, some offset will be required.\\

2. \textbf{ Conveyor belt (B1) to conveyor belt (B2) transformation:} \\
For this part, it is quite simple. This transformation is about the movement of the conveyor belt from the Camera’s Field of view to the Robot’s Workspace/Pick range. The only value that change in this transformation is only the X-axis value. As per mentioned in the introduction, we also include the encoder’s reading to helps us track how much the distance has the conveyor belt covered.\\
...
Please be patient with me while I go through this lengthy and complicated section. The "3-Points Calibration Method" must be used in order to obtain a homogenous transformation matrix between the robot frame and the conveyor belt (B2). This approach is regarded reliable and is frequently utilized in belt tracking systems. The remaining information on this approach may be found here and here. In essence, when the robot points toward three distinct spots on the conveyor belt, you must record the coordinate values of the robot base frame.\\
...
After the object leaves and is picked up by the exit photoeye, we will record the count. We'll use 6311 as the number. Therefore, we will deduct 5232 from 6311 to get the count of the entire voyage, which comes out to be 1079 counts.
...
The main Vision camera we have used to track the moving object's motion through the conveyor belt is a Photoneocamera (MotionCam 3D M+), which has advanced settings that can capture and detect the object's position.\\\\ The photoneo camera mainly consists of 3D sensing technology, which contains parallel structured light that helps provide the light source to detect the objects. .\newpage  Using this camera, the camera can capture accurate point clouds and a standard intensity image of the object. \\\
