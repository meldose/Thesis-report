...
\end{center}
    ...
    \vspace{0.5 cm}
     ...
     The Deggendorf Institute of Technology supervises an examination paper with the topic of\textbf{ Object Tracking using 3D Machine Vision Camera for Indsutrial Robots}
...
The figure below shows the S30 calibration board that helps calibrate the conveyor belt, which aligns the coordinates of the object with the conveyor belt for objects passing through the belt.\\
...
The trajectory follows the instructions produced from trajectory planning and eye-in-hand vision each time the robot arm's palm begins to perform a moving task. The border of the conveyor will show up on the top side of the image when viewed through a camera. Because the conveyor's moving path is linear, the robot's motion can be corrected by following the conveyor's edge's slope.The line equation in polar coordinates to determine the conveyor's line edge. The polar parametric form has the advantage of having no singular condition in the domain of sinusoidal functions.
...
The servoX is the motion of the robot in Cartesian space, where we need to define the max velocity, max accelerationa and its servoX propotional gain. Initially the coordinates that we get from the camera space as Quaternion values that include [X,Y,Z ,ex,ey,ez] as the target values for the robot to move that particular position and pick up the object. Since we have calibrated camera with respect to the Robot frame, the coordinates that we get the from the object will be with the respect to the robot frame.
...
\begin{enumerate}
 ...
 If the localized calibration ball was successfully located for the extrinsic calibration, it is indicated with a green highlight.  On the other hand, that portion of texture will be highlighted in red if there is insufficient ball surface visible or if another item with a similar shape is found.
...
\begin{enumerate}
    ...
    \item The Add calibration point request can be called by the robot.
\end{enumerate}
...
Three calibration sites are needed for both the hand-eye and extrinsic calibration.  The resulting calibration matrix and calibration accuracy information are only available once the necessary number of calibration points has been added.
...
In the below figure, the number represents the coordinates of the object after the scanning of the object done after the trigger. Once these quaternion values are attained, these values are then sent to the robotic controller for the robot to move to the required target position.\\\\
...
Initally before we start the calibration we need to set the camera to an specific profile setup to Marker Space according to calibration setup and save the profile as given below:
...
This method can also be used to get the Conveyor belt to be calibrated. The steps include :
\begin{enumerate}
    \item \textbf{First step:}  Place the picking / triggering object in the field of view of the camera and trigget the camera , then get the object pose whcih include the [X1,Y1, and Z1] respectively
    \item \textbf{Second step:} Switch on the conveyor belt and ge the farest postion where the camera trigger the object and get the coordinates which include the [X2,Y2 and Z3] respectively.
    \item \textbf{Third step:} While triggering the camera in the first and final positon we also try to capture the timestamps of the camera, which calculates the time taken by the camera to trigger.
    \item \textbf{Fourth step:}  In this step , we need to caculate the change in the position for the X,Y and Y repsectively which includes ($\Delta X$, $\Delta Y$, and $\Delta Z$ ) respectively. In addition to that we need to get the change in the time also  ($\Delta t$).
    \item \textbf{Fifth step:}  In this step we need to find the custom velocity by diving the change in the postion by the change in the time.
...
\\ The benefit of analytical expression is that it provides formulas for the connection between link parameters and joint angle. The robotics controller can directly incorporate these relationships.\\
...
\begin{enumerate}
    ...
    \item If not assert an issue showing that there is an unexpected issue.
    ...
    \item Create the target position with the quaternion values that received form the camera
    \item Check he condition where the object needs to be picked, if distance is less that 700 then dot the motion of move linear ()
...
\end{enumerate}
\newpage
The below figure shows the flowchart for the Dynamic Picking:
...
\begin{figure}[h]
    \centering
    \includegraphics[width=3.5cm]{Thesis-report/Figures/network_interface.jpeg}
...
Gigabit Ethernet cables (Cat5e or higher category) are needed to connect the 3D sensor.  A gigabit switch can be used to connect several 3D sensors.  In all situations, the switch or 3D sensor's Ethernet connection is attached to the Scanner network port.
\begin{figure}[h]
    \centering
    \includegraphics[width=3.5cm]{Thesis-report/Figures/scanner_interface.jpeg}
    \caption{Robot Network Interface}
    \label{fig1.Photoneo Cmaera}
\end{figure}
 ...
 Both a fixed IPvL link-local address and a programmable IPv4 address are features of Photoneo 3D Sensors.IPvL connections are better than IPv4 ones.  Nevertheless, the IPv4 address is used in the event that the IPvL connection is banned or fails.  Consequently, having a legitimate IPv4 network configuration is advised.\\
...
The robot connection status is currently indicated by the following indicator:
 ...
 Action Request Client: the state of the Action Request Communication Channel connection between the Action Request Server and Action Request Client.  It may exist in one of two states:
 \begin{enumerate}
     \item  \textbf{CONNECTED:} The Action Request Client is able to send requests since it is connected to the Action Request Server.\\ 
...
The Robotic movement can be with 3 methods called Servo X , ServoJ and Movelinear method.\
\begin{itemize}
    \item ServoX
    \item Servo J
    \item Movelinear
\end{itemize}\\
1. \textbf{ServoX:} In the Servo X motion, we move the robot in cartesian motion, where the coordinates that we get from the camera are in quaternion poses [X, Y, Z, Qx, Qy, Qy, W]. These are already in the Cartesian poses, which can be directly passed to the robotic controller, giving these values as the target position, and moving the robot to the desired location. During servoX, the coordinates that came from the camera were not in the correct order as that of the robotic movement, so initially the target position had to be changed as in this form [X, Y, Z, W, Qx, Qy, Qy]\\\\
2. \textbf{ServoJ:} In the servo J , the move of the robot is with the joint movements, rather than the Cartesian. The object coordinates that we get from the camera are in Cartesian form, they need to be converted into joint form and given as the target position. When giving the target position, the final orientation values of the coordinates will not be the same as that of the TCP coordinates of the robot. In order to set this, we have to implement the method inverse kinematics.\\\\
3. \textbf{Movelinear:}
The Movelinear is the method in which the target position for the robot to pick up the object is directly given as the quaternion values provided by the camera by scanning the objects. Here we define the maximum velocity, acceleration . The movement of the robot happens in an servo interface where passing the quaternion values to the robot as the target postion which helps the robot to the required postion.

\subsection{Static Picking:}\\
Static pick-up is the process of picking items from the conveyor belt when it is not moving. Initially we keep the object in the conveyor belt and then trigger the camera that helps in getting the object coordinates to the robotic controller. When the robot receives the data, the robot moves to the target position to pick the required target position.
\subsection{Dynamic Picking:}\\
The Dynamic Pick involves the process of picking the items from the moving conveyor belt. Here the object is passed through the moving conveyor bet where the camera gets object poses that are passed to the robotic controller, which helps the robot move to the required position.\\

For the Dynamic picking a typical construction site is needed, quick real-time modeling is necessary to support decision-making.  Thus, a quick 3D data collection and processing system is required for real-time modeling of a building site.  Only high-quality, rich 3D datasets of static objects are produced using laser scanners; a typical scan has millions of points.  Furthermore, real-time modeling is not practicable due to the slowness of data collecting and processing.  Nevertheless, 3D range cameras provide an additional option by enabling wide-field-of-view, low-cost, automatic tracking and detection of both static and moving objects at frame rates higher than 1Hz (real-time).
The working happens in Dynamic can be as follows:\\

Initally we pass the pobject through the running conveyor belt and trigger the camera , where we get thr intinal and final coordinates. We also calcualte the timestamps also to get the chanegs in the time also. Then we need to find the custom velocity , which can be calculated by dividing the change in the positon and changes in time .
When we pass the object through the conveyor belt , there is change in position in the X axis as this is an linear motion of movement . The change in the postion of the object need to be trasnfered to the robot , so that robot can understand where should i need to come and pick the item from the conveyor belt. This can be compensated by using the following equation: \[ X_0 = X + v \cdot t \] where X0 is the change in the position of the object. \\
wherer X is the intial postion of the object where the object gets triggered\\
v si the velocity / calibrated custom velocity in m/s\\
t is change in time \\

\subsection{Coordinate Transformation:}
Many applications in manufacturing, construction, and vehicle automation and autonomy depend on the availability of 3D information.  Time-of-flight (TOF) sensors are getting cheaper and more accessible. They offer depth information at every pixel in addition to intensity.
The TOF sensor provides spherically-coordinated range data of the environment within its field of view.  As described in the following section, the spherical coordinates are transformed into Cartesian coordinates, with the origin of the coordinate system located at the camera's center.
 ...
 \end{enumerate}
    \item The z-axis is normal to the belt plane in the belt coordinate system.  By referring points above the belt plane (z > 0) and inside the belt limits, this alignment of the coordinate axis makes it easier to extract item point clouds on the belt.
\subsection{Euler Angles:}\\
Euler angles can be used to parameterize the space of orientations. A universal orientation is expressed as a sequence of rotations along three mutually orthogonal axes in space when Euler angles are applied. In a Cartesian coordinate system, the x, y, and z axes are typically utilized. The rotations are frequently referred to as x-, y-, and z-rolls.In order to solve differential equations, Euler initially created Euler angles. The most popular technique for parameterizing the space of orientations is now the use of later Euler angles.Euler angles can be used to parameterize the space of rotations if we decide to think of a rotation as the activity carried out to achieve a specific orientation. The rotation angles about the x, y, and z axes are denoted as { 1, { 2, and { 3, respectively.
...
The figure above shows the STL file of the trapezoid, which is used to pick up the object. While loading the STL file for the object, we will get the X,Y,Z coordinates for the object.
...
\begin{figure}[h]
    \centering
{
        \includegraphics[width=3cm]{Thesis-report/Figures/CV belt1.jpeg}
        \label{fig:cv_belt1}
    }
    \quad
{
        \includegraphics[width=3cm]{Thesis-report/Figures/CV_ belt2.jpeg}
        \label{fig:cv_belt2}
    }
...
1. \textbf{Conveyor Belt:} The conveyor belt is the main surface that moves materials or products from one point to another. It runs always when the motor is switched on and carry the items of various shapes and sizes effectively.\\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=6cm]{Thesis-report/Figures/cv2belt.jpg}
...
\begin{figure}[h]
    ...
    \label{fig1.Photoneo Cmaera}
\end{figure}
Runtime requests from action clients are received and executed following the deployment of a defined solution.  The Action Request Client (Robot main software) and Photoneo 3D Sensors (vision systems) connection statuses can be viewed by the user.Additionally, it provides the ability to call a Scan request and Get object request, as well as to halt or resume the deployed solution.\\
...
\end{figure}
\newpage
 The above figure represents the web interface that includes the camera triggering the object placed in the workspace. The green section represents the area for the workspace, and the red colored is the object to be picked up by the Robot. Using an TCP/IP client request connection in which the robot and camera connection is being established. For setting up the connection the Robotic controller`s and camera IP address (subnet) are made into same , which helps in communication through web interface.

 The above figure shows the volume/area in which the Photoneo Camera is possible to scan the items and get the object poses. The top view shows the maximum FOV where the camera can reach the object to get the object coordinates, this includes 15m.
\begin{figure}[h]
    \centering
    \includegraphics[width=6cm]{Thesis-report/Figures/scanning volume.png} 
    \caption{Scanning Volume}
    \label{fig1.Photoneo Cmaera}
\end{figure}
 \newpage
\chapter{\section{\mbox{Results and Discussions }}{{\normalfont\fontsize{14}{16}\bfseries}}
The below figure shows the results that is obtained while pciking the items in an moving conveyor belt speed (m/s) with respect to Robot speed.\\
\label{Literature}
\begin{figure}[h]
    \centering
    ...
    \label{fig1.Photoneo Cmaera}
\end{figure}\\
The below figure shows the results that are obtained while placing the items in a moving conveyor belt speed (m/s) with respect to Success rates.\\
\begin{figure}[h]
    \centering
...
\begin{figure}[h]
    ...
    \label{fig1.Photoneo Cmaera}
\end{figure} \\
The below figure shows the results obtained while placing the items in a moving conveyor belt speed (m/s) with respect to the Avg pick up time.\\
\begin{figure}[h]
    \centering
