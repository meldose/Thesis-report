...
Surface imaging and geometry estimation are the two primary subtasks that comprise imaging of the item to be inspected.Basically, there are numerous ways to implement automated optical inspection systems in the field of optical inspection.  A good combination of strategies should be chosen based on the task at hand.Improving the contrast between an object's flawed and perfect areas is crucial for dependable and robust automated detection systems in the field of optical surface inspection.  In particular, the lighting and optics define the contrasts.
...
\end{figure}\\
\textbf{2.EventMap:}
The Event Map helps in capturing objects in fast motion without any blur.It helps in capturing the full frames under a fixed rate.
    \begin{figure}[h]
    \centering
...
The trajectory follows the instructions produced from trajectory planning and eye-in-hand vision each time the robot arm's palm begins to perform a moving task. The border of the conveyor will show up on the top side of the image when viewed through a camera. Because the conveyor's moving path is linear, the robot's motion can be corrected by following the conveyor's edge slope. The line equation in polar coordinatesis used to determine the conveyor's line edge. The polar parametric form has the advantage of having no singular condition in the domain of sinusoidal functions.
...
\begin{figure}[h]
    ...
    \caption{Calibration for camera concerning Robot frame}
    \label{fig1.Photoneo Cmaera}
\end{figure}
